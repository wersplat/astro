...
export async function fetchTeamsData(searchParams: SearchParamsType, cookieStore: any) {
  const supabase = createClientServer(cookieStore);
  const page = parseInt(searchParams.page || '1');
  const offset = (page - 1) * ITEMS_PER_PAGE;
  
  // Build query using team_performance_by_game_year like the existing JS
  let query = supabase
  ...
  if (searchParams.search) {
    query = query.ilike('team_name', `%${searchParams.search}%`);
  }

  // Filter for active teams only by joining with teams table
  ...
  const sort = searchParams.sort || 'rp-desc';
  switch (sort) {
    case 'rank-asc':
    case 'rp-desc':
  ...
  let totalPages = Math.ceil((count || 0) / ITEMS_PER_PAGE);
  ...
  if (!teamPerformanceData || teamPerformanceData.length === 0) {
    ...
    if (searchParams.search) {
      teamsQuery = teamsQuery.ilike('name', `%${searchParams.search}%`);
    }
    ...
    // Build synthetic performance list so downstream logic stays consistent
